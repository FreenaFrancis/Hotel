...
//           ...others,
//           cheapestPrice: { $gt: min | 1, $lt: max || 999 }, // Use || instead of |
...
const createHotel = async (req, res, next) => {
  ...
  try {
    const savedHotel = await newHotel.save();
    res.status(201).json(savedHotel); // 201 Created for successful creation
  } catch (err) {
    next(err);
  }
};
...
const updateHotel = async (req, res, next) => {
  try {
    ...
    if (!updatedHotel) {
      return next(createError(404, "Hotel not found")); // 404 Not Found for missing resource
    }
    res.status(200).json(updatedHotel);
  } catch (err) {
    next(err);
